...
\begin{enumerate}
	...
	Because this is a semester project there is a hard deadline, where the development has to end.
	\item Pass on project \label{enum:passed}\\
	This project is meant to be passed on to the students next year.
	...
	\item No manager/Product owner \label{enum:manager}\\
	...
	Although this is a multi-project, we are divided in four groups each group with their own working room.
	\item Low criticality \label{enum:criticality}\\
	If the system fails the failure will not influence the real world much.
\end{enumerate}
