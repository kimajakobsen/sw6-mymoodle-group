...
If all the features in the sprint backlog are not implemented during the sprint they are moved to the \productbacklog{}, which is a list of the features that should be implemented in the future.
...
\Sos{}~\cite{scrumOfScrums} dictates that there should be a \scrum{} master in each sub-group.
...
This may not be ideal, but we consider it to be more important that every member of the subgroups tries to have the responsibility of a \scrummaster{} than having only one member trying it and learning it well.
